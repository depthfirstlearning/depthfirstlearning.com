...
observing each variable separately. 
  ...
  **Questions**:
  1. From PRML: 1.31, 1.36, 1.37, 1.38, 1.39, 1.41.
     ...
     <p>
     PRML 1.31 Consider two variables x and y having joint distribution p(x, y). Show that the differential entropy of this pair of variables satisfies H(x,y) <= H(x) + H(y) with equality if, and only if, x and y are statistically independent.
     ...
     No mutual information if \(p(x)\) and \(p(y)\) are independent: \(I(x,y) = H(x) + H(y) - H(x,y)\) because \(H(x,y) = H(x) + H(y)\)
     </p>
...
     </p><p>
     The lower bound is reached when P(x) = Q(x):
     \(D_{KL}(P||Q) =  \sum_{x}P(x) log_2\left(\frac{P(x)}{Q(x)}\right) = \sum_{x}P(x) log_2(1) = \sum_{x}P(x) 0 = 0\)
     </p><p>
     The upper bound is reached when P(x) is maximally different from Q(x):
     Then the cross entropy is infinite, because the number of bits needed to encode the signal would be infinite.
     \(H_P(Q) = \infty \to D_{KL}(P||Q) =  H_P(Q) - H(Q) = \infty\)
     \(H_P(Q) = - \sum_{x} Q(x) log_2(P(x)) = \infty \to D_{KL}(P||Q) = H_P(Q) - H(Q) = \infty\)
     </details>
